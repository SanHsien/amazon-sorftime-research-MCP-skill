...
- **治理決策**：
  1. 對外只打主人的 repo（`SanHsien/amazon-sorftime-research-MCP-skill`），嚴禁自動向上游開 PR 或 push。
...
### 1. 繁體中文與語系重整
- **README 重構**：
  - `README.md` 改以繁體中文（台灣跨境電商習慣用語）為主。
  - 保留英文版為 `README.en.md`。
...
- **#1 1688成本获取失败**：第三方供應鏈 API 介面逾時或 Cookie 需更新，本 fork 保留原邏輯並於相關 script 加入容錯。
- **#2 咨询个问题** / **#5 大佬 我想问个问题**：一般使用問題，無需代碼變更。
- **#3 加一个 product-research Skill 的快速体验入口**：上游已在後續 commit 補齊 product-research。
- **#4 VOKO 项目自荐**：外部項目推廣，不予採納。
- **#6 大佬，codex要怎么用这套skill啊**：本 fork 完善了 Codex 與 Claude Code 的整合指引，支援在 `.codex` / Skills 目錄下使用。
